...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  ...
  if (!match) return { h: 205, s: 100, l: 68 }; // Default to brand sky blue HSL
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  return (
    ...
    >
      {/* mesh gradient border (handles conic-gradient cursor sweep) */}
      <div
      ...
      >
        {/* actual border line (masked to exclude the inside padding-box completely) */}
        <div
          className="absolute inset-0 rounded-[inherit]"
          style={{
            border: '1px solid transparent',
            background: borderBg.map(g => `${g} border-box`).join(', '),
            WebkitMask: 'linear-gradient(#fff 0 0) padding-box, linear-gradient(#fff 0 0) border-box',
            WebkitMaskComposite: 'destination-out',
            mask: 'linear-gradient(#fff 0 0) padding-box exclude, linear-gradient(#fff 0 0) border-box',
